...
public class TbReplyService implements DService {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			int tbGroup = Integer.parseInt(tbRequest.getParameter("tbGroup"));
			int tbStep = Integer.parseInt(tbRequest.getParameter("tbStep"));
			int tbIndent = Integer.parseInt(tbRequest.getParameter("tbIndent"));
			TalkMeBoardDao tbDao = new TalkMeBoardDao();
			
			int result = tbDao.tbReply(cId, tbSubject, tbContent, tbFileName01, tbFileName02, tbFileName03, tbFileName04, tbFileName05, tbIp, tbGroup, tbStep, tbIndent);
			if(result == TalkMeBoardDao.SUCCESS) {
				request.setAttribute("resultMsg", "답변성공");
			}else {
				request.setAttribute("resultMsg", "답변실패");
